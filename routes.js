...
export const setupRoutes = (fastify) => {
    fastify.get('/api/search', async (req, reply) => {
        ...
        try {
            ...
            return reply.send({ success: true, contentType: 'device_details', data: data });
        } catch (error) {
    ...
    fastify.get('/api/compare', async (req, reply) => {
        ...
        try {
            ...
            return reply.send({ success: true, contentType: 'comparison', data: data });
        } catch (error) {
            return reply.status(500).send({ success: false, error: error.message });
        } finally {
            if (browser) await browser.close();
    ...
    fastify.get('/api/suggestions', async (req, reply) => {
        ...
        try {
            ...
            const formattedResults = results.map((r, i) => {
                const slug = r.slug || r.url_name || r.url || r.link || r.alias || r.id || r.name?.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
                return {
                    index: i,
                    name: r.name,
                    type: r.content_type,
    ...
    fastify.get('/api/rankings', async (req, reply) => {
        ...
        try {
            ...
            return reply.send({ success: true, contentType: 'rankings', data: data });
        } catch (error) {
            return reply.status(500).send({ success: false, error: error.message });
        } finally {
            if (browser) await browser.close();
