 ...
 * 1. Memory cache                  → <1ms
 * 2. Next.js /_next/data/ JSON API → ~150-300ms  ← pure JSON, no parsing
 ...
 * we have a slug from the top-priority type search.
 */
...
/**
 * detectTypes — returns prioritized type list based on query keywords.
 * Returns ALL_TYPES for ambiguous/short/generic queries.
 */
const detectTypes = (query) => {
    const q = query.toLowerCase().trim();

    // Short or generic terms: search everything to avoid missing results
    if (q.length <= 7 && !q.includes(' ')) {
        return { top: ALL_TYPES, rest: [] };
    }

    if (/ryzen|intel|core i[3579]|threadripper|xeon|celeron|pentium/i.test(q))
        return { top: ['cpu', 'laptop'], rest: ['soc', 'phone', 'tablet', 'gpu'] };
    if (/snapdragon|mediatek|exynos|dimensity|a[0-9]{2}\s*bionic|helio/i.test(q))
        return { top: ['soc', 'phone'], rest: ['tablet', 'laptop', 'cpu', 'gpu'] };
    if (/nvidia|rtx|gtx|radeon|rx\s*[0-9]|geforce|arc\s*a[0-9]/i.test(q))
        return { top: ['gpu', 'laptop'], rest: ['cpu', 'phone', 'tablet', 'soc'] };
    if (/iphone|galaxy\s*s|pixel\s*[0-9]|oneplus|xiaomi|redmi|poco|realme|samsung|huawei|oppo|vivo|nokia|motorola|sony\s*xperia/i.test(q))
        return { top: ['phone', 'soc'], rest: ['tablet', 'laptop', 'cpu', 'gpu'] };
    if (/ipad|galaxy\s*tab|surface\s*pro|tab\s*s[0-9]/i.test(q))
        return { top: ['tablet', 'phone'], rest: ['soc', 'laptop', 'cpu', 'gpu'] };
    if (/macbook|thinkpad|xps|zenbook|vivobook|chromebook|ultrabook/i.test(q))
        return { top: ['laptop', 'cpu'], rest: ['gpu', 'tablet', 'phone', 'soc'] };

    // Unknown/generic query — search all types
    return { top: ALL_TYPES, rest: [] };
};
...
function dedupe(results, query) {
    ...
    return results
        .filter(r => {
            const k = r.slug || r.url_name || r.url || r.id || r.name;
            if (seen.has(k)) return false;
            seen.add(k);
            return true;
        })
        .sort((a, b) =>
            score(b.name, b.slug || b.url_name || '', query) -
            score(a.name, a.slug || a.url_name || '', query)
        );
}
...
export const searchAndFetch = async (query, limit = 10) => {
    ...
    const allTypes = [...new Set([...top, ...rest])];

    // For broad queries (top === ALL_TYPES) just fire one search; otherwise fire two in parallel
    const isAllTypes = top.length === ALL_TYPES.length;
    const allPromise = tryDirectSearch(query, limit, allTypes, cookies);
    const topPromise = !isAllTypes ? tryDirectSearch(query, limit, top, cookies) : null;

    let earlyFetchPromise = null;
    let earlyItem = null;

    // Use top results for early pre-fetch if we have a narrower type list
    if (topPromise) {
        ...
        if (topResults?.length) {
            ...
            if (earlyItem) {
                ...
                console.log(`[pipeline] top hit → pre-fetching ${earlyItem.name}`);
    ...
    const merged = dedupe([...(allResults || [])], query);
    let finalItem;
    ...
    if (!merged.length) {
        ...
        if (bCached) {
            bCached.searchResults = fmtResults(bDeduped);
            bCached.matchedDevice = finalItem.name;
            return bCached;
        }
        const bHtml = await browserFetchDirect(bUrl);
        const bData = parseDeviceHtml(bHtml, bUrl);
        bData.searchResults = fmtResults(bDeduped);
        bData.matchedDevice = finalItem.name;
    ...
    if (finalCached) {
        finalCached.searchResults = fmtResults(merged);
        finalCached.matchedDevice = finalItem.name;
        return finalCached;
    }
    ...
    if (earlyFetchPromise && earlyItem && deviceUrl(earlyItem) === finalUrl) {
        ...
        if (earlyFetchPromise) earlyFetchPromise.catch(() => {});
...
function parseDeviceHtml(html, url) {
    if (!html) return { title: '', sourceUrl: url, images: [], scores: {}, pros: [], cons: [], specs: {} };

    // Fast regex path — extract __NEXT_DATA__ without full DOM parse
    try {
    ...
    $('.card,.box,section,[class*="specs"]').each((_, card) => {
        ...
        $(card).find('table tr').each((__, row) => {
            ...
            if (cells.length >= 2) {
                const l = cells.first().text().trim().replace(/:$/, '');
                const v = cells.last().text().trim();
                if (l && v && l !== v) section[l] = v;
            }
        });
...
export const searchDevices = async (query, limit = 10) => {
    ...
    const { top, rest } = detectTypes(query);
    const allTypes = [...new Set([...top, ...rest])];
    let results = await tryDirectSearch(query, limit, allTypes, cookies);
    if (!results?.length) results = await browserSearchDirect(query, limit, allTypes);
    const deduped = dedupe(results || [], query);
    if (deduped.length) cache.set('search', cacheKey, deduped, TTL.search);
    return deduped;
};
...
export const scrapeDevicePage = async (url) => {
    ...
    if (!data) {
        try {
            const html = await directFetchHtml(url, cookies, 5000);
            if (html) data = parseDeviceHtml(html, url);
        } catch {}
    }
    if (!data) {
        const html = await browserFetchDirect(url);
        data = parseDeviceHtml(html, url);
    }
...
export const scrapeComparePage = async (compareUrl) => {
    ...
    const cookies = await getCFCookies() || '';
    let html;
    try { html = await directFetchHtml(compareUrl, cookies, 5000); } catch {}
...
export const scrapeRankingPage = async (rankingUrl) => {
    ...
    const cookies = await getCFCookies() || '';
    let html;
    try { html = await directFetchHtml(rankingUrl, cookies, 5000); } catch {}
